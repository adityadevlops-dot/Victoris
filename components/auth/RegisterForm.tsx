...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // TODO: Implement actual JWT registration logic
    setTimeout(() => {
      setIsLoading(false);
    ...
    >
      ...
      <div className="mb-10 text-center">
        ...
        <p className="font-['Rajdhani'] text-[#a1a1aa] text-lg font-medium">
          Create your gladiator identity and enter the arena.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        >
          ...
          <span className="relative z-10">{isLoading ? "Forging Identity..." : "Create Account"}</span>
        </button>
      </form>
      ...
      <div className="mt-10 text-center border-t border-[#1a1a1a] pt-8">
        <p className="font-['Rajdhani'] text-[#a1a1aa] text-lg">
          Already a gladiator?{" "}
          <Link href="/login" className="text-[#fafafa] hover:text-[#dc2626] transition-colors font-bold tracking-wide">
            Enter Arena
          </Link>
        </p>
      </div>
